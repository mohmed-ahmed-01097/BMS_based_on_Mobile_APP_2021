...
void __interrupt() ISR(void){
    ...
    if(INTCONbits.T0IF){                    // CHECK RC INTERRUPT FLAG
        ...
        if((++TIME_MS)>=1000){              // CHECK TIME_MS 
            ...
            FREQUENCY_ARRAY[TIME_S%INDEX] = ZERO_CROSS; 
...
void Send(void){
    ...
    switch(CHAR_STATE){                             // UART STATE CHAR 
        case UART_TOGGLE:
            ...
            UART_Send(((PORTC&0X08)>>3),MEAS_S );   // UART SEND
            CHAR_STATE = 0;                         // RESET THE UART STATE CHAR
            break;                            
        case UART_SEND:
            UART_Send(F_VALUE           ,MEAS_F );  // UART SEND F
            UART_Send(V_VALUE           ,MEAS_V );  // UART SEND V
            UART_Send(A_VALUE           ,MEAS_A );  // UART SEND A
            UART_Send(PF_VALUE          ,MEAS_PF);  // UART SEND PF
            UART_Send(((PORTC&0X08)>>3),MEAS_S );   // UART SEND
            CHAR_STATE = 0;                         // RESET THE UART STATE CHAR
            break;
    }
