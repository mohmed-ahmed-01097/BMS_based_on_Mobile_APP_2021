...
void EEPROM_Init(S_MyData *_data){
  ...
  _data->Energy = EEPROM_Monitor_Read_H(_data->Now.Hr);
...
// Power EEPROM
// >
//Writing EEPROM Measuring Energy
//Stack Pointer : 222
void EEPROM_Monitor_Write_H(const uint16_t _index, const float _data){
  
  Serial.print  (micros()); Serial.println( "\tMPWH_S");
  Serial.println("Writing EEPROM Measuring Energy");
  Serial.println("Stack Pointer : " + String(_index*4 + EEPROM_HOUR_S));  
  EEPROM.put(_index*4 + EEPROM_HOUR_S,_data);
  Serial.print  (micros()); Serial.println( "\tMPWH_E"); 
...
void EEPROM_Monitor_Write_D(const uint16_t _index, const uint8_t _end){
  ...
  float ret = 0;
  Serial.print  (micros()); Serial.println( "\tMPWD_S");
  Serial.println("Writing EEPROM Measuring Energy");
  for(uint8_t i=0 ; i<=_end ; i++){
    Serial.println("Stack Pointer : " + String(i*4 + EEPROM_HOUR_S)); 
    ret += EEPROM_Monitor_Read_H(i);
  } 
  EEPROM.put(_index*4 + EEPROM_DAY_S, ret);
  Serial.print  (micros()); Serial.println( "\tMPWD_E"); 
...
void EEPROM_Monitor_Write_M(const uint16_t _index, const uint8_t _end){
  ...
  float ret = 0;
  Serial.print  (micros()); Serial.println( "\tMPWM_S");
  Serial.println("Writing EEPROM Measuring Energy");
  for(uint8_t i=0 ; i<=_end ; i++){
    Serial.println("Stack Pointer : " + String(i*4 + EEPROM_DAY_S)); 
    ret += EEPROM_Monitor_Read_D(i);
  } 
  EEPROM.put(_index*4 + EEPROM_MONTH_S, ret);
  Serial.print  (micros()); Serial.println( "\tMPWM_E"); 
...
// Power EEPROM
// >82.288 msec
//Reading EEPROM Measuring Energy Hour
//Stack Pointer : 222  nan
float EEPROM_Monitor_Read_H(const uint16_t _index){
  
  float ret = 0;
  Serial.print  (micros()); Serial.println( "\tMPRH_S");
  Serial.println("Reading EEPROM Measuring Energy Hour");
  Serial.print  ("Stack Pointer : " + String(_index*4 + EEPROM_HOUR_S) + "\t");  
  EEPROM.get(_index*4 + EEPROM_HOUR_S, ret);
  Serial.println("H[" + String(_index*4 + EEPROM_HOUR_S) + "]\t" + ret);
  Serial.print  (micros()); Serial.println( "\tMPRH_E"); 
  return (ret>0) ? ret : 0;
...
// Power EEPROM
// >82.288 msec
//Reading EEPROM Measuring Energy Day
//Stack Pointer : 318 nan
float EEPROM_Monitor_Read_D(const uint16_t _index){
  
  float ret = 0;
  Serial.print  (micros()); Serial.println( "\tMPRD_S");
  Serial.println("Reading EEPROM Measuring Energy Day");
  Serial.print  ("Stack Pointer : " + String(_index*4 + EEPROM_DAY_S) + "\t");  
  EEPROM.get(_index*4 + EEPROM_DAY_S, ret);
  Serial.println("D[" + String(_index*4 + EEPROM_DAY_S) + "]\t" + ret);
  Serial.print  (micros()); Serial.println( "\tMPRD_E"); 
  return (ret>0) ? ret : 0;
...
// Power EEPROM
// >82.288 msec
//Reading EEPROM Measuring Energy Mounth
//Stack Pointer : 442  nan
float EEPROM_Monitor_Read_M(const uint16_t _index){
  
  float ret = 0;
  Serial.print  (micros()); Serial.println( "\tMPRM_S");
  Serial.println("Reading EEPROM Measuring Energy Mounth");
  Serial.print  ("Stack Pointer : " + String(_index*4 + EEPROM_MONTH_S) + "\t");  
  EEPROM.get(_index*4 + EEPROM_MONTH_S, ret);
  Serial.println("M[" + String(_index*4 + EEPROM_MONTH_S) + "]\t" + ret);
  Serial.print  (micros()); Serial.println( "\tMPRM_E"); 
  return (ret>0) ? ret : 0;
