...
void WiFi_Init(void){
  ...
  //WiFi_CreatWebServer();                                                        // Check the WiFi 
  //server.begin();
}
...
void WiFi_SetupMode(void){
  ...
  if(WiFi.status() == WL_CONNECTED) {                                   // well connected
    delay(5000);
    server.reset();     // remove all rewrites, handlers and onNotFound/onFileUpload/onRequestBody callbacks
  ...
  if(E_SSID == "⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮⸮"){
    E_SSID  = "ESP_" + String(ESP.getChipId());
    E_PASS  = "12345678";
    AP_SSID = "ESP_" + String(ESP.getChipId());
  }else{
...
void WiFi_CreatWebServer(void){
  ...
  server.on("/Measure", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/" + String(menu[1]) + ".html", String(), false, processor);
  });
  // Route for root / web page
  ...
  server.on("/jquery.plugin.js", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/jquery.plugin.js", "text/javascript");
  });
...
/***********************************************************************************************************************************************/    
  ...
  server.on("/Voltage", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Measure.V).c_str());
  });
  server.on("/Current", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Measure.A).c_str());
  });
  server.on("/Frequency", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Measure.F).c_str());
  });
  server.on("/PowerFactor", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Measure.PF).c_str());
  });
  server.on("/ActivePower", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Power.WATT).c_str());
  });
  server.on("/ReactivePower", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Power.VAR).c_str());
  });
  server.on("/Energy", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(_DATA->Energy).c_str());
  });

  server.on("/Hours", HTTP_GET, [](AsyncWebServerRequest *request){
    String ret = Monitor_Graph_H();
    Serial.println(ret);
    request->send_P(200, "text/plain", String(ret).c_str());
  });
  server.on("/Days", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", Monitor_Graph_D().c_str());
  });
  server.on("/Months", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", Monitor_Graph_M().c_str());
  });
/***********************************************************************************************************************************************/    
